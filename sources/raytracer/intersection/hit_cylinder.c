...
/*   Updated: 2025/12/16 23:04:42 by eraad            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include <minirt.h>

static void	set_cylinder_record(t_cylinder *cylinder, t_ray *ray,
		t_hit_record *record, t_cylinder_vars *vars)
{
	...
	record->hit_point = vec3_add(ray->origin, vec3_scale(ray->direction,
				record->t));
	axis_point = vec3_add(cylinder->center, vec3_scale(cylinder->axis,
				vars->projection));
...
t_bool	hit_cylinder(t_object *object, t_ray *ray, t_hit_record *record)
{
	...
	if (solve_quadratic(&vars.eq_vars) == FALSE)
		return (FALSE);
	if (is_valid_height(&vars, vars.eq_vars.root1))
		record->t = vars.eq_vars.root1;
	else if (is_valid_height(&vars, vars.eq_vars.root2))
