...
/*   Updated: 2025/12/17 13:38:31 by eraad            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		t_cylinder_vars *vars)
{
	...
	t = vec3_dot(vec3_sub(vars->bottom_center, ray->origin),
			vec3_scale(object->u_data.cylinder.axis, -1.0)) / denominator;
	if (t < ray->min || t > ray->max || t >= record->t)
		return ;
	p = vec3_add(ray->origin, vec3_scale(ray->direction, t));
	if (vec3_len_squared(vec3_sub(p,
				vars->bottom_center)) > (object->u_data.cylinder.radius_squared))
		return ;
	record->t = t;
	record->hit_point = p;
