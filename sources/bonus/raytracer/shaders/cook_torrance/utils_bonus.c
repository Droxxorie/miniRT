...
/*   Updated: 2026/01/29 09:31:20 by eraad            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		t_ray *ray, int depth)
{
	...
	v->n_dot_v = fmaxf(vec3_dot(v->n, v->v), 0.0);
...
t_real	distribution_ggx(t_vec3 n, t_vec3 h, t_real roughness)
{
	...
	n_dot_h = fmaxf(vec3_dot(n, h), 0.0);
...
t_real	geometry_smith(t_vec3 n, t_vec3 v, t_vec3 l, t_real roughness)
{
	...
	n_dot_v = fmaxf(vec3_dot(n, v), 0.0);
	n_dot_l = fmaxf(vec3_dot(n, l), 0.0);
