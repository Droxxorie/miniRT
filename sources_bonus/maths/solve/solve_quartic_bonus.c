...
/*   Updated: 2026/01/09 16:02:53 by eraad            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	normalize_coeffs(t_quartic *vars)
{
	...
	vars->a = vars->coeffs[0] * a_inv;
	vars->b = vars->coeffs[1] * a_inv;
	vars->c = vars->coeffs[2] * a_inv;
	vars->d = vars->coeffs[3] * a_inv;
}
...
static t_bool	compute_ferrari_vars(t_quartic *vars)
{
	...
	vars->e = sqrt(fmax(t1 + t2, 0.0));
	vars->f = sqrt(fmax(t1 - t2, 0.0));
	return (TRUE);
}

static void	set_quartic_roots(t_quartic *vars)
{
	vars->roots[0] = -vars->a / 4.0 + (vars->r_var + vars->e) / 2.0;
	vars->roots[1] = -vars->a / 4.0 + (vars->r_var - vars->e) / 2.0;
	vars->roots[2] = -vars->a / 4.0 - (vars->r_var + vars->f) / 2.0;
	vars->roots[3] = -vars->a / 4.0 - (vars->r_var - vars->f) / 2.0;
	vars->roots_count = 4;
}
