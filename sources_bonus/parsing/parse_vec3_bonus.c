...
/*   Updated: 2026/01/02 16:42:49 by eraad            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_status	parse_axis(t_scene *scene, char **line, t_vec3 *axis)
{
	...
	if (axis->x < -1.0 || axis->x > 1.0)
		return (print_error_loc(scene, start_line, ERR_UX_OOR), 1);
	if (parse_comma(scene, line) == EXIT_FAILURE)
		return (1);
	start_line = *line;
	...
	if (axis->y < -1.0 || axis->y > 1.0)
		return (print_error_loc(scene, start_line, ERR_UY_OOR), 1);
	if (parse_comma(scene, line) == EXIT_FAILURE)
		return (1);
	start_line = *line;
	...
	if (axis->z < -1.0 || axis->z > 1.0)
		return (print_error_loc(scene, start_line, ERR_UZ_OOR), 1);
