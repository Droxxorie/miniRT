...
SRCS_BONUS += \
	...
	$(SRC_DIR_BONUS)/raytracer/engines/path_tracing/bsdf/sample_rough_dielectric_bonus.c \
...
valgrind: $(NAME)
	...
	@valgrind --leak-check=full --show-leak-kinds=all \
		...
		./$(NAME) $(ARGS) > valgrind.out 2>&1 || true

helgrind: $(NAME)
	@echo "$(YELLOW)${BOLD}[HG] ./$(NAME) $(ARGS)$(RESET)"
	@valgrind --tool=helgrind \
		--suppressions=test/valgrind.supp\
		./$(NAME) $(ARGS) > helgrind.out 2>&1 || true

valgrind_bonus: $(NAME_BONUS)
	@echo "$(YELLOW)${BOLD}[VG] ./$(NAME_BONUS) $(ARGS)$(RESET)"
	@valgrind --leak-check=full --show-leak-kinds=all \
		--track-origins=yes --suppressions=test/valgrind.supp\
		./$(NAME_BONUS) $(ARGS) > valgrind.out 2>&1 || true

helgrind_bonus: $(NAME_BONUS)
	@echo "$(YELLOW)${BOLD}[HG] ./$(NAME_BONUS) $(ARGS)$(RESET)"
	@valgrind --tool=helgrind \
		--suppressions=test/valgrind.supp\
		./$(NAME_BONUS) $(ARGS) > helgrind.out 2>&1 || true
...
fclean: clean
	...
	@if [ -f valgrind.out ] ; then echo "$(RED)[RM] valgrind.out$(RESET)" ; fi
	@if [ -f helgrind.out ] ; then echo "$(RED)[RM] helgrind.out$(RESET)" ; fi
	@rm -f $(NAME) $(NAME_BONUS) valgrind.out helgrind.out
...
help:
	...
	@echo "$(BRIGHT_MAGENTA)helgrind  →  ${RESET}Check for threading issues."
	@echo "$(BRIGHT_MAGENTA)helgrind_bonus →  ${RESET}Check for threading issues in bonus version."
...
#* ---- Phony -------------------------------------------------------------------
.PHONY: all clean fclean re re_bonus bonus help valgrind valgrind_bonus helgrind helgrind_bonus